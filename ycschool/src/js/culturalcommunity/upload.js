...
$.fn.bindUpload = function () {
    ...
    this.openWindow = function (files) {
        ...
        var box = '<div id="zwb_upload_status">' +
            '<div class="box">' +
            '        <div class="title">' +
            '            共' + Math.min(files.length,myData.num) + '个文件&nbsp;' + ((allSize / 1048576).toFixed(3) == 0 ? allSize / 1048576 : (allSize / 1048576).toFixed(3)) + 'mb' +
            '        </div>' +
            '        <div class="list">' +
            '<ul class="listBox"></ul>' +
            '</div>' +
        ...
        _this.find(".off").bind("click", function () {
            ...
            status = [];
            files = null;
            var $list=$('<div class="feimgList"></div>');
            var imgArr=photoArray.split('|');
            for(var i=0;i<imgArr.length-1;i++){
                $list.append('<img src="'+ SERVERROOTFILE + imgArr[i] +'">');
            }
            if(photoArray!==''){
                $('.feorgan-certificate').html($list);
            }
    ...
    this.getImgSrc = function (files) {
        ...
        if (type == "png" || type == "jpg" || type == "gif" || type == "svg" || type == "jpeg") {
    ...
    this.upload = function (max, i, item) {
        ...
        formData.append("action", "formDataUploadNew");
