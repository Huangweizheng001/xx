	...
	w.CHAT = {
		...
		init: function(username, userImg, userGrade, ipAddress, fromUrl, keyWord, mid, channelId, noteInf) {
			...
			*/
			...
			this.formType = "moblile Web"; //来自于移动web端 app this.formType = "App";
			this.scrollToBottom();
...
$(function() {
	...
	function getAword(){
		$.ajax({ //get a word information
			type: "get",
			url: SERVEROOTDATA + "Technology.ashx?action=getAword",
			dataType: 'json',
			data: {},
			success: function(res) {
				var obj = res[0];
				$("#jbmwordTitle1").text(obj.titleLabel);
				$("#jbmwordTitle2").text(obj.title);
				
				$("#jbmwordBtn").on("click",function(){
	...
	function ckdetail() {
		...
		$('#livetab').on('click','.swiper-pagination-bullet',function(){
			...
			if(nowindex == 0){
				window.location.href = 'login.html?live=1';
			}else if(nowindex == 2){
				window.location.href = 'teacherdetail.html?teacherId=' + teacherId + '&live=1';
			}else if(nowindex == 3){
				if(isbookfirst) {
					courseBook();
				}
