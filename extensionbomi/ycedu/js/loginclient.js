...
//			return false;
		...
		init2: function(username, icon) {
			...
			*/
			...
			//监听微信用户登录
			this.socket.on('loginMessage', function(ssid,obj) {
				if(ssid == sid){
					//this.socket.emit("")
					$.thirdLoginFunc(obj.memberId, obj.name, obj.nickName, obj.iconPath, obj.memberlevel, obj.sowingCoin);
				}
			});
			...
			*/
			...
			$.loginType = function(type){
				$.ajax({
					...
					success: function(res) {
						console.log(res[0])
						$("#jbmloginImg").attr("src",ROUTEFILE+res[0].codePath);
					}
				});
			}
			
			var weixinLoginType = $(this).getUrlParam("login");
			if( weixinLoginType == 2){
				weixinLoginType = 1;
