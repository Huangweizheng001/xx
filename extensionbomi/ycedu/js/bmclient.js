			...
			*/
			...
			this.socket.on("loginToAdmin", function(obj, type) {
				...
				if(type == 1) {
					...
					section.innerHTML = '<div class="dycadmin-admin-img"><img src="./images/live/systemUser.png"></div><div class="dycadmin-box"><div class="dycadmin dycadmin-admin"><span class="bmsendMessageTime">' + timeStampToTime(obj.loginTime) + '</span></div>' + uName + ' : 关闭注册登录框</div>';
					...
					var section = d.createElement('div');
					section.className = "dycdiscuss-box-admin bmdiscuss-system-box";
					section.innerHTML = '<div class="dycadmin-admin-img"><img src="./images/live/systemUser.png"></div><div class="dycadmin-box"><div class="dycadmin dycadmin-admin"><span class="bmsendMessageTime">' + timeStampToTime(obj.loginTime) + '</span></div>' + uName + ' : 正在注册</div>';
					CHAT.msgObj.appendChild(section);
					setTimeout(function() {
						$(".bmmessagePanel").mCustomScrollbar("scrollTo", $("#message").height());
					}, 200)
					...
					var section = d.createElement('div');
					section.className = "dycdiscuss-box-admin bmdiscuss-system-box";
					section.innerHTML = '<div class="dycadmin-admin-img"><img src="./images/live/systemUser.png"></div><div class="dycadmin-box"><div class="dycadmin dycadmin-admin"><span class="bmsendMessageTime">' + timeStampToTime(obj.loginTime) + '</span></div>' + uName + ' : 关闭对话框</div>';
					CHAT.msgObj.appendChild(section);
					setTimeout(function() {
						$(".bmmessagePanel").mCustomScrollbar("scrollTo", $("#message").height());
					}, 200);
	...
	function getUserInf(mid,flag) {
		$.ajax({
			...
			success: function(res) {
				...
				$.ajax({
					type: "get",
					url: ROUTE + "Channel.ashx?action=saveChannelPswByMemberId",
					dataType: 'json',
					data: {
						channelId:6,
						memberId:$mid,
	...
	function createPlayer(URL, URL2) {
		...
		if(cIP == "::ffff:218.6.69.201" || cIP == "::ffff:110.90.7.92") {
			var objectPlayer = new aodianPlayer({
				...
				adveDeAddr: './images/live/bglw01.jpg', //封面图片链接
				//adveWidth: 320,//封面图宽度
...
Btn").on("click", function() {
	...
	$("#bmgetTicket,#jbmbuyTicket").on("click", function() {
		if($mid == null || $mid == undefined || $mid == "undefined" || $mid == "") {
			layer.open({
				type: 2,
				title: "登录观看讲座",
				...
				maxmin: true, //开启最大化最小化按钮
				area: ['700px', '580px'],
				content: './zt/1122pay.html?from=yhlive'
			});
		} else {
			layer.open({
				type: 2,
	...
	$("#jbmhadCode").on("click",function(){
		layer.prompt({title: '输入播放码，并确认', formType: 2}, function(pass, index){
			$.ajax({
				type: "get",
				url: ROUTE + "Channel.ashx?action=saveChannelPswByMemberId",
				dataType: 'json',
				...
				timeout: 300,
				success: function(res) {
					if(res.code==7901 || res.code == "7901"){
						layer.msg("播放码错误!");
					}else if(res.code =="200" || res.code == 200){
						layer.msg("播放码校验成功！");
						layer.close(index);
						if(!hasPlayer){
							$("#dbLivePlayer").html('<div class="kzvisiterBox"></div>');
	...
	$("#jstudyToolBox .bmstudyToolBook").on("click", function() {
		if($mid == null || $mid == undefined || $mid == "undefined" || $mid == "") {
			...
			layer.open({
				...
				maxmin: true, //开启最大化最小化按钮
				area: ['800px', '560px'],
				content: './mynote.html?chId=8&pId=' + currentProgramId + '&tId=' + teacherId
			});
		}
	})
})
